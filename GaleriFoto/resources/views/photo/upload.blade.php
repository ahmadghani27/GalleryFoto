 ...
 <form action="{{ route('foto.singleupload') }}" method="POST" enctype="multipart/form-data" x-data="{ submitting : false }" @submit.prevent="submitting = true; $el.submit()">
     ...
     <div class="self-stretch flex flex-col justify-start items-start gap-3">
         ...
         <div class="drop-zone self-stretch p-5 rounded-2xl outline outline-1 outline-offset-[-1px] outline-black/5 flex justify-between items-center">
             ...
             <label class="w-48 h-14 px-2.5 py-5 rounded-2xl outline outline-[1.5px] outline-offset-[-1.5px]  hover:bg-cyan-600 outline-neutral-900 flex justify-center items-center gap-2.5 cursor-pointer transition-all duration-100 hover:bg-neutral-900 hover:text-white">
     ...
     <div class="self-stretch flex flex-col justify-start items-start gap-6">
         <div class="self-stretch flex flex-col justify-start items-start gap-3">
             ...
             <div class="self-stretch bg-zinc-100 rounded-2xl relative flex items-center justify-center overflow-y-hidden overflow-x-auto drop-zone">
                 <div id="preview-container" class="w-full flex justify-center items-center min-h-40"></div>
                 <div class="absolute w-40 h-40 border-2 border-dashed border-black/50 rounded-lg pointer-events-none"></div>
             </div>
             <input type="file" id="photo-upload" accept="image/jpeg,image/png" multiple class="hidden" onchange="handleFileUpload(event)">
         </div>

         <!-- Title Input -->
         <div class="self-stretch flex flex-col justify-start items-start gap-3">
             <label class="self-stretch text-black text-base font-medium">Judul Foto<span class="text-yellow-400">*</span></label>
             ...
             if (['image/jpeg', 'image/png'].includes(file.type)) {
                 ...
                 reader.onload = (e) => {
                     ...
                     img.onload = function() {
                         ...
                         const isSmallerThan40 = this.height <= 160 && this.width <= 160;

                         if (isSmallerThan40) {
                             // Ukuran kecil: paksakan tinggi 40 dan lebar auto
                             imgElement.className = 'h-40 w-auto object-contain';
                         } else if (isLandscape) {
                             // Landscape: max-height 40, lebar auto
                             imgElement.className = 'max-h-40 w-auto object-contain';
                         } else if (isPortrait) {
                             // Portrait: max-width 40, tinggi auto
                             imgElement.className = 'max-w-40 h-auto object-contain';
                         } else {
                             // Kotak atau seimbang
                             imgElement.className = 'max-w-40 max-h-40 object-contain';
                         }
